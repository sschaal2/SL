...
static char f_cond_names[][20] = { {"dummy"}, {"full"}, {"half"}, {"orth"}, {"no_used"}};
...
// local functions
static void computeContactForces(ObjectPtr optr,
                                 ContactPtr cptr);
static void contactVelocity(int cID,
                            ObjectPtr optr,
                            double *v);
static void addObjectSync(char *name,
                          int type,
                          int contact,
                          double *rgb,
                          double *pos,
...
static void changeHideObjByNameSync(char *name,
                                    int hide);
static void changeObjPosByNameSync(char *name,
                                   double *pos,
                                   double *rot);
static void convertGlobal2Object(ObjectPtr optr,
                                 double *xg,
                                 double *xl);
static void computeStart2EndNorm(double *xs,
                                 double *xe,
                                 ObjectPtr optr,
                                 double *n);
static void projectForce(ContactPtr cptr,
                         ObjectPtr optr);
 ...
 ******************************************************************************/
ObjectPtr addCylinder(char *name,
                      double *rgb,
                      double *pos,
                      double *rot,
                      double *scale,
 ...
 ******************************************************************************/
int changeObjPosByName(char *name,
                       double *pos,
                       double *rot)
{
 ...
 ******************************************************************************/
int changeHideObjByName(char *name,
                        int hide)
{
  ObjectPtr ptr;
  int i;
 ...
 ******************************************************************************/
int getObjForcesByName(char *name,
                       double *f,
                       double *t)
{
  ObjectPtr ptr;
 ...
 ******************************************************************************/
int getObjForcesByPtr(ObjectPtr ptr,
                      double *f,
                      double *t)
{
  int i;
 ...
 ******************************************************************************/
void changeObjPosByPtr(ObjectPtr ptr,
                       double *pos,
                       double *rot)
{
  int i;
...

{
  ...
  { /* loop over all contact points */
    ...
    { /* check all objects */
      ...
      {
        case CUBE: //---------------------------------------------------------------
          if (fabs(x[1]) < optr->scale[1] / 2. && fabs(x[2]) < optr->scale[2] / 2. && fabs(x[3]) < optr->scale[3] / 2.)
          {
            ...
            if (first_contact_flag)
            {
              ...
              if ((optr->scale[1] / 2. - fabs(x[1])) < (optr->scale[2] / 2. - fabs(x[2])) && (optr->scale[1] / 2. - fabs(x[1])) < (optr->scale[3] / 2.
                  - fabs(x[3])))
              {
                ind = 1;
              }
              else if ((optr->scale[2] / 2. - fabs(x[2])) < (optr->scale[1] / 2. - fabs(x[1])) && (optr->scale[2] / 2. - fabs(x[2])) < (optr->scale[3] / 2.
                  - fabs(x[3])))
              {
                ind = 2;
              }
            ...
            for (j = 1; j <= N_CART; ++j)
            {
              if (j == ind)
              {
                ...
                contacts[i].normal[j] = optr->scale[j] / 2. * macro_sign(contacts[i].x_start[j]) - x[j];
        ...
        case SPHERE: //---------------------------------------------------------------
          if ((sqr(x[1] / optr->scale[1] * 2.) + sqr(x[2] / optr->scale[2] * 2.) + sqr(x[3] / optr->scale[3] * 2.)) < 1.0)
          {
        ...
        case CYLINDER: //---------------------------------------------------------------
          ...
          if ((sqr(x[1] / optr->scale[1] * 2.) + sqr(x[2] / optr->scale[2] * 2.)) < 1.0 && fabs(x[3]) < optr->scale[3] / 2.)
          {
    ...
    { // this is just for easy data interpretation
      for (j = 1; j <= N_CART; ++j)
      {
        contacts[i].normal[j] = 0.0;
        contacts[i].normvel[j] = 0.0;
                         ...
                         ObjectPtr optr)
{
  ...
  /***********************************************************************************************/
  if (cptr->n_connected_links == 0)
  { // this is a interpolated contact point or a special contact point

    if (cptr->point_contact_flag)
    { // for special contact points
      double xs[N_CART + 1];
      double xe[N_CART + 1];
    ...
    { // for interpolated line contact points

      // compute norm vector from start to end of line
      computeStart2EndNorm(link_pos_sim[cptr->id_start], link_pos_sim[cptr->id_end], optr, nv);
  ...
  { // this is a contact point coinciding with a link point
    ...
    for (i = 1; i <= cptr->n_connected_links; ++i)
    {
      ...
      switch (cptr->force_condition[i])
      {
        ...
        case F_HALF: // project away force aligned with line, pointing out of line
          ...
          // compute norm vector from start to end of line
          computeStart2EndNorm(link_pos_sim[cptr->connected_links[i]], link_pos_sim[cptr->id_start], optr, nv);
...

{
  ...
  while (fscanf(in, "%s %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %d", name, &objtype, &rgb[1], &rgb[2], &rgb[3], &pos[1], &pos[2], &pos[3], &rot[1],
                &rot[2], &rot[3], &scale[1], &scale[2], &scale[3], &contact) == 15)
  {
    ...
    while ((string[i++] = fgetc(in)) != '\n' && i < 499)
      ;
    string[i] = '\0';
    i = sscanf(string, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf ", &oparms[1],
               &oparms[2], &oparms[3], &oparms[4], &oparms[5], &oparms[6], &oparms[7], &oparms[8], &oparms[9], &oparms[10], &oparms[11], &oparms[12],
               &oparms[13], &oparms[14], &oparms[15], &oparms[16], &oparms[17], &oparms[18], &oparms[19], &oparms[20], &oparms[21], &oparms[22], &oparms[23],
               &oparms[24], &oparms[25], &oparms[26], &oparms[27], &oparms[28], &oparms[29], &oparms[30]);
    oparms[0] = i;
    ...
    while ((string[i++] = fgetc(in)) != '\n' && i < 499)
      ;
    string[i] = '\0';
    i = sscanf(string, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf ", &cparms[1],
               &cparms[2], &cparms[3], &cparms[4], &cparms[5], &cparms[6], &cparms[7], &cparms[8], &cparms[9], &cparms[10], &cparms[11], &cparms[12],
               &cparms[13], &cparms[14], &cparms[15], &cparms[16], &cparms[17], &cparms[18], &cparms[19], &cparms[20], &cparms[21], &cparms[22], &cparms[23],
               &cparms[24], &cparms[25], &cparms[26], &cparms[27], &cparms[28], &cparms[29], &cparms[30]);
    cparms[0] = i;

    if (objtype == TERRAIN)
    { // terrains use .asc names
    ...
    if (objtype == TERRAIN)
    {
      if ((ID = getNextTerrainID()))
      {
        ...
        setTerrainInfo(ID, ID, name, pos, q.q, (int)rint(oparms[1]), (int)rint(oparms[2]), (int)rint(oparms[3]));
...

{
  ...
  switch (optr->contact_model)
  {
    ...
    case DAMPED_SPRING_STATIC_FRICTION:
      ...
      for (i = 1; i <= N_CART; ++i)
      {
        cptr->f[i] = optr->contact_parms[1] * cptr->normal[i] + optr->contact_parms[2] * cptr->normvel[i];
      ...
      for (i = 1; i <= N_CART; ++i)
      {
        temp[i] = -optr->contact_parms[3] * cptr->tangent[i] - optr->contact_parms[4] * cptr->tanvel[i];
  ...
  ucontact[cptr->base_dof_start].t[_A_] += moment_arm[_Y_] * cptr->f[_Z_] * cptr->fraction_start - moment_arm[_Z_] * cptr->f[_Y_] * cptr->fraction_start;
  ucontact[cptr->base_dof_start].t[_B_] += moment_arm[_Z_] * cptr->f[_X_] * cptr->fraction_start - moment_arm[_X_] * cptr->f[_Z_] * cptr->fraction_start;
  ucontact[cptr->base_dof_start].t[_G_] += moment_arm[_X_] * cptr->f[_Y_] * cptr->fraction_start - moment_arm[_Y_] * cptr->f[_X_] * cptr->fraction_start;

  /* get the torque at the object center from the cross product */
  optr->t[_A_] += moment_arm_object[_Y_] * cptr->f[_Z_] * cptr->fraction_start - moment_arm_object[_Z_] * cptr->f[_Y_] * cptr->fraction_start;
  optr->t[_B_] += moment_arm_object[_Z_] * cptr->f[_X_] * cptr->fraction_start - moment_arm_object[_X_] * cptr->f[_Z_] * cptr->fraction_start;
  optr->t[_G_] += moment_arm_object[_X_] * cptr->f[_Y_] * cptr->fraction_start - moment_arm_object[_Y_] * cptr->f[_X_] * cptr->fraction_start;
  ...
  /* get the torque at the DOF from the cross product */
  ucontact[cptr->base_dof_end].t[_A_] += moment_arm[_Y_] * cptr->f[_Z_] * cptr->fraction_end - moment_arm[_Z_] * cptr->f[_Y_] * cptr->fraction_end;
  ucontact[cptr->base_dof_end].t[_B_] += moment_arm[_Z_] * cptr->f[_X_] * cptr->fraction_end - moment_arm[_X_] * cptr->f[_Z_] * cptr->fraction_end;
  ucontact[cptr->base_dof_end].t[_G_] += moment_arm[_X_] * cptr->f[_Y_] * cptr->fraction_end - moment_arm[_Y_] * cptr->f[_X_] * cptr->fraction_end;

  /* get the torque at the object center from the cross product */
  optr->t[_A_] += moment_arm_object[_Y_] * cptr->f[_Z_] * cptr->fraction_end - moment_arm_object[_Z_] * cptr->f[_Y_] * cptr->fraction_end;
  optr->t[_B_] += moment_arm_object[_Z_] * cptr->f[_X_] * cptr->fraction_end - moment_arm_object[_X_] * cptr->f[_Z_] * cptr->fraction_end;
  optr->t[_G_] += moment_arm_object[_X_] * cptr->f[_Y_] * cptr->fraction_end - moment_arm_object[_Y_] * cptr->f[_X_] * cptr->fraction_end;
                            ...
                            double *v)
{
  ...
  computeLinkVelocity(contacts[cID].id_start, link_pos_sim, joint_origin_pos_sim, joint_axis_pos_sim, joint_sim_state, v_start);
  computeLinkVelocity(contacts[cID].id_end, link_pos_sim, joint_origin_pos_sim, joint_axis_pos_sim, joint_sim_state, v_end);
                                   ...
                                   double *rot)
{
  ...
  if (servo_time - last_update_pos_time > 1. / 60.)
  { // 60Hz update is sufficient
...

{
  ...
  while (TRUE)
  {
    ...
    if (rc == 6)
    { // add the appropriate number of contact points
      ...
      { // point contacts are isolated and do not fill the connect_links array
        ...
        if (!f_full_flag)
        { // a full force condition overwrites any partial force conditions
          if (fcond1ID == F_FULL)
          {
 ...
 ******************************************************************************/
void computeContactPoint(ContactPtr cptr,
                         double *x)

{
  ...
  { // a line contact
    for (j = 1; j <= N_CART; ++j)
      x[j] = (link_pos_sim[cptr->id_start][j] * cptr->fraction_start + link_pos_sim[cptr->id_end][j] * cptr->fraction_end);
