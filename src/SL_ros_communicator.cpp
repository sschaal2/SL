...
bool SL_ros_communicator::publish()
{
	...
	for(int i=1; i<=n_dofs; ++i) {
		joint_state_msg.name[i-1] = joint_names[i];
		joint_state_msg.position[i-1] = joint_state[i].th;
		joint_state_msg.velocity[i-1] = joint_state[i].thd;
		joint_state_msg.effort[i-1] = joint_state[i].u;
	}
	nrt_joint_state_publisher_.publish(joint_state_msg);

	// publish endeffector pose in NON REAL-TIME
	geometry_msgs::PoseStamped pose_stamped_msg;
	pose_stamped_msg.header.seq = 1;
	pose_stamped_msg.header.stamp = ros::Time::now();
	int i=1;
	pose_stamped_msg.pose.position.x = cart_state[i].x[_X_];
	pose_stamped_msg.pose.position.y = cart_state[i].x[_Y_];
	pose_stamped_msg.pose.position.z = cart_state[i].x[_Z_];
