 ...
 ******************************************************************************/
void addToUserGraphics(char *abr,
                       char *string,
                       void(*fptr)(void *),
                       int n_bytes)
{
...
int activateUserGraphic(char *name)
{
  ...
  while (ptr != NULL)
  {
    ...
    {
      // user graphics was successfully identified
      ptr->active = TRUE;
      break;
    }
  ...
  if (ptr == NULL)
    printf("Couldn't find user grpahic >%s<\n", name);
  ...
  for (i = 1; i <= sm_user_graphics->n_entries; ++i)
  {
    ...
    while (ptr != NULL)
    {
      ...
      if (strcmp(ptr->abr, name) == 0)
      {
        ...
        memcpy(ptr->buf, sm_user_graphics->buf + sm_user_graphics->moff[i], sizeof(unsigned char) * ptr->n_bytes);
...
static void displayBall(void *b)
{
  ...
  glPushMatrix();
  glTranslated((GLdouble)ball[_X_], (GLdouble)ball[_Y_], (GLdouble)ball[_Z_]);
